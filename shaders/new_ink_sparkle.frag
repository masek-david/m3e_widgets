...
/// thickness is 0.05 * max_radius
/// radius is actually current_radius, which is max_radius * progress
...
float sparkle(vec2 uv, float t) {
  ...
  s += threshold(n + sin(PI * (t + 1.05)), 0.3, 0.35);
                              // EDIT replace 0.55 with u_sparkle_alpha
  return clamp(s, 0.0, 1.0) * u_sparkle_alpha;
}
...
void main() {
  ...
  vec2 uv = p * u_resolution_scale;
                            // EDIT replace p with uv
  vec2 density_uv = uv - mod(uv, u_noise_scale);
